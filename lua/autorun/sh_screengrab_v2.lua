...
if CLIENT then
	...
	local quality
	local _ply
	local render_Capture=render.Capture -- breaking render.Capture is a common way to prevent screengrabbing
	local util_Base64Encode=util.Base64Encode -- breaking util.Base64Encode is a common way to prevent screengrabbing
	local util_Compress=util.Compress -- breaking util.Compress is another possible way to prevent screengrabbing
	...
	local net_func=function()
		net_Receive("StartScreengrab",function()
			...
			for name,func in SortedPairs(hook_GetTable().PostRender) do
				if func()!=nil then--if something is returned here, it probably means someone is trying to bypass us
	...
	local timer_NAME=math.Rand(0,1000).."screengrab"..math.Rand(0,1000).."timer"
